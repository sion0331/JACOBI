...
from utils.symbolic_utils import create_variable, is_redundant, is_redundant_optimized
...
def generate_new_population(history, population, config):
    ...
    for system in sorted_population[:num_parents]:
        ...
        systems_map.append([sorted([get_term_id(term) for term in eq]) for eq in system])
    ...
    while n < n_crossover:
        ...
        s_map = [sorted([get_term_id(term) for term in eq]) for eq in child]
    ...
    print(f"systems_map:{len(systems_map)}, term_map:{len(get_term_map())}")
